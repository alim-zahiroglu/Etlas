...
function showModal() {
    // Retrieve data from the <td> element
    const salesPrice = document.getElementById('showUpdatePriceBox').innerText.trim();
    const currencySymbol = document.getElementById('showUpdatePriceBox').querySelectorAll('span')[1].innerText.trim();

    // Populate modal fields with the retrieved data
    document.getElementById('perchesPrice').querySelector('.currency-symbol').innerText = currencySymbol;
    document.getElementById('perchesPrice').querySelector('input').value = salesPrice;
